...
env = { 'sqrt': torch.sqrt,
        ...
        'uniform-continuous': distG.Uniform,
        'exponential': dist.Exponential,
        'beta': dist.Beta,
        'discrete': distG.Categorical,
        'dirichlet': distG.Dirichlet,
        'flip': distG.Bernoulli,
        'gamma': distG.Gamma,
        'dirac': None,
        'vector': torch.stack,
...
def deterministic_eval(exp,sig,v):
    "Evaluation function for the deterministic target language of the graph based representation."
    if type(exp) is list:
        op = env[exp[0]]
        args = [deterministic_eval(i,sig,v) for i in exp[1:]]
        return funcprimitives(exp[0],op,args,sig,v)
...
def funcprimitives(e,op,args,sig,v):
	if e == 'vector':
		try:
			ret = op(args)
		except:
		...
		Dobj = args[0]
		ret = args[1]
		sig['logW'] = sig['logW']+Dobj.log_prob(ret).detach().nan_to_num()
	elif e == 'if':
		if args[0]:
			ret = args[1]
		else:
