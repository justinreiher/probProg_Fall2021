...
def evaluate_program(e,sig):
    """Evaluate a program as desugared by daphne, generate a sample from the prior
    Args:
        ast: json HOPPL program
    ...
    if type(e) == int or type(e) == float:
        ...
        testExp,sig = evaluate_program(e[1],sig)
        if testExp:
            ret,sig = evaluate_program(e[2],sig)
        else:
            ret,sig = evaluate_program(e[3],sig)

    elif e[0] == 'sample':
        addr = e[1]
        d = e[2] # get the distribution declaration
        if e[1] not in sig:
            Dobj,sig = evaluate_program(d,sig)
            sig[e[1]] = Dobj
            ret = sig[e[1]].sample()
        else:
            ret = sig[e[1]].sample() #sample from that distribution object with the associated parameters
        ...
        try: #first try to get a distribution object from the known distributions directly
            ...
            Dobj,sig = evaluate_program(d,sig) #assign the distribution object
            ret,sig = evaluate_program(y,sig)
        ...
        for i in range(1,len(e)):
            r,sig = evaluate_program(e[i],sig) #only interested in the returned expression
